...
public:
    ...
    int getTotalNodos()
    {
        return getTotalNodos(root);
    }

    int getMinStock() {
        NodoArbolAVL<T> *minNode = findMin(root);
        if (minNode) {

            return minNode->getData().getStock();
        }
        return -1;
    }

    int getStock(const std::string& data) {
        return getStockSum(data, root);
    }

    int getStockSum(const std::string& data, NodoArbolAVL<T>* r) {
        if (r == nullptr)
            return 0; // Si el nodo es nulo, la suma es 0.

        if (data == r->getData()) {
            // Encontramos el producto, sumamos la cantidad en este nodo y
            // continuamos buscando en los subárboles izquierdo y derecho.
            int leftSum = getStockSum(data, r->getLeft());
            int rightSum = getStockSum(data, r->getRight());
            return leftSum + rightSum + r->getData().getStock();
        } else if (data < r->getData()) {
            // Si el producto buscado es menor que el actual, buscamos en el subárbol izquierdo.
            return getStockSum(data, r->getLeft());
        } else {
            // Si el producto buscado es mayor que el actual, buscamos en el subárbol derecho.
            return getStockSum(data, r->getRight());
        }
    }

    int countAllNodes() {
        return getTotalNodos();
    }

    int getMaxStock() {
        NodoArbolAVL<T> *maxNode = findMax(root);
        if (maxNode) {
            return maxNode->getData().getStock();
        }
        return -1;
    }
    ...
    NodoArbolAVL<T> *getRoot();

    int getBalance()
    {
        return getBalance(root);
    }
    void put(T data, int stock);
private:
    ...
    NodoArbolAVL<T> *put(T data, NodoArbolAVL<T> *r, string nom);
    NodoArbolAVL<T> *remove(T data, NodoArbolAVL<T> *r);

    int getTotalNodos(NodoArbolAVL<T> *r)
    {
        if (r == nullptr)
            return 0;

        return getTotalNodos(r->getLeft()) + getTotalNodos(r->getRight()) + 1;
    }
...
template <class T>
NodoArbolAVL<T> *getMinNode(NodoArbolAVL<T> *r)
{
    if (r == nullptr)
        return nullptr;
    if (r->getLeft() == nullptr)
        return r;
    return getMinNode(r->getLeft());
}

template <class T>
NodoArbolAVL<T> *getMaxNode(NodoArbolAVL<T> *r)
{
    if (r == nullptr)
        return nullptr;
    if (r->getRight() == nullptr)
        return r;
    return getMaxNode(r->getRight());
}
...
template <class T>
NodoArbolAVL<T> *ArbolBinarioAVL<T>::getRoot(){
    return root;
}

template <class T>
NodoArbolAVL<T> *ArbolBinarioAVL<T>::put(T data, NodoArbolAVL<T> *r, string nom)
{
  //1. Realizar Insercion normal
  if (r == nullptr)
  {
    return new NodoArbolAVL<T>(data,nom);
  }
  ...
  if (r->getData() > data)
  {
    r->setLeft(put(data, r->getLeft(),nom));
  }
  else
  {
    r->setRight(put(data, r->getRight(), nom));
  }
...
template <class T>
void ArbolBinarioAVL<T>::put(T data, int stock){
    root = put(data, root, stock);
};
